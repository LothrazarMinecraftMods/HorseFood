...
public class ItemHorseFood extends Item{
	...
	public static void onHorseInteract(EntityHorse horse, EntityPlayer player, ItemStack held){
		...
		if(held.getItem() == ItemRegistry.emeraldCarrot){
			...
			int var_new = 0;
			while (var - 256 > 0){
				var_reduced += 256;// this could be done with modulo % arithmetic too, but meh
									// doesnt matter either way
				var -= 256;
			} // invalid numbers make horse invisible, but this is somehow safe. and easier than
				// doing bitwise ops
			switch(var){
			...
			if(ModHorseFood.horseJumpStrength != null)// only happpens if mod installing preInit
